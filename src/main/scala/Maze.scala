...
case class Sounds(audioSynth: Option[AudioSynth]) {
  ...
  def blipDown() =
    audioSynth.foreach(_.blip(400, 100, 20, 100))
  def sweepUp() =
...
class MazeModel(sounds: Sounds) {
  ...
  def solveMazeDFS(update: => Unit): Unit = {
    ...
    try {
      def doNextCell(c: Cell): Unit = {
        c.visited = true
        c.trail = Forward
        if (c.getDirections().isEmpty) blipDown()
        c.getDirections().foreach { dir =>
          getCell(c, dir).foreach { n =>
            if (n.visited != true) {
              ...
              update
              beep()
              if (n.isAt(exit)) throw new Exception("Done")
    ...
    try {
      while (!q.isEmpty) {
        ...
        if (c.visited != true) {
          ...
          if (c.isAt(exit)) throw new Exception("Done")
  ...
  def showSolution(update: => Unit): Unit = {
    ...
    val cell: Option[Cell] = Some(cellAt(exit))
  ...
  def cellAt(p: Point) = cells(p.x)(p.y)
...
// a scala.swing.Panel, override paintComponent(Graphics2D) to paint each of the cells
...
case class Cell(i: Int, j: Int) {
  ...
  def draw(g2: Graphics2D): Unit = {
    val bounds = g2.getClipBounds()
    val size = Math.min(bounds.width / WIDTH, bounds.height / HEIGHT)
    val x = bounds.x + i * size
    val y = bounds.y + j * size

    def fillCell() =
      g2.fillRect(x + cellBorder, y + cellBorder, size - cellBorder * 2, size - cellBorder * 2)
    ...
    if (north) g2.drawLine(x, y, x + size, y)
    if (south) g2.drawLine(x, y + size, x + size, y + size)
    if (east) g2.drawLine(x + size, y, x + size, y + size)
    if (west) g2.drawLine(x, y, x, y + size)
  }
